...
struct line {
    ...
    int   tstate;
    unsigned tcmd;
    int   after_cr;             /* только что отдали возврат каретки */
...
static int feed_byte(struct line *L, unsigned c)
{
    ...
    if (L->mode == M_RAW) {
        /* Одно нажатие Enter — один байт, даже если устройство прислало два.

           NVT из RFC 854 требует посылать возврат каретки как CR NUL, а
           клиенты в строчном режиме шлют CR LF; и то и другое — одна клавиша.
           Полноэкранная программа, читающая байты, видела два нажатия и
           делала два действия: в `mc` Enter заходил в каталог и тут же
           возвращался из него. Программа была права, считая и 13, и 10 за
           Enter, — знание о том, что устройство шлёт их парой, принадлежит
           терминалу, и вот оно. */
        if (L->after_cr && (c == 0 || c == '\n')) {
            L->after_cr = 0;
            return 0;
